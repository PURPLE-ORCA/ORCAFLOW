    ...
    (id) => {
      ...
      return columnIds.find(
        (key) =>
          columns[key] &&
          columns[key].items &&
          Array.isArray(columns[key].items) &&
          columns[key].items.some((item) => getItemValue(item) === id)
      );
    },
  ...
  const handleDragEnd = React.useCallback(
    (event) => {
      ...
      if (onMove && !isColumn(active.id)) {
        console.log("🚀 [KANBAN DEBUG] Calling onMove with event:", {
          active,
          over,
        });
      ...
      if (
        ...
        const activeIndex = columns[container].items.findIndex(
          (item) => getItemValue(item) === active.id
        );
        const overIndex = columns[container].items.findIndex(
          (item) => getItemValue(item) === over.id
        );
...
function KanbanItem({ value, asChild = false, className, children, disabled }) {
  ...
  return (
    ...
    >
      <Comp {...mergedProps}>{children}</Comp>
    </ItemContext.Provider>
  );
}
...
function KanbanColumnContent({ value, className, children }) {
  ...
  const itemIds = React.useMemo(
    () => (columns[value].items || []).map(getItemId),
  ...
  React.useEffect(() => {
    if (activeId) {
      ...
      if (element) {
        ...
        console.log(
          "🚀 [KANBAN DEBUG] Overlay element not found for selector:",
          `[data-slot="kanban-${
            isColumn(activeId) ? "column" : "item"
          }"][data-value="${activeId}"]`
        );
